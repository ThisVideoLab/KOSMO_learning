...
public class CreateListObject {
	...
	public static void main(String[] args) {
		...
		// List는 인터페이스 이므로 new 키 뒤에 올수 없다. ArrayList,Vector,LinkedList
		//capacity : 메모리 방의 크기를 할당, 기본값은 10
		// size() : 방에 저장된 값의 갯수
		
		List<Integer> aList = new ArrayList<>();	//cppacity = 10
		List<Integer> aList2 = new ArrayList<>(30); //cppacity = 30, 메모리 공간 확보.
...
//		List<Integer> aList6 = new LinkedList(30);  //LinkedList는 저장공간 할당 불가.
		...
		//2. Arrays 클래스의 정적 메소드를 이용 하는 방법 :데이터 크기를 바꿀수 없다
			//요일, 게정 ,하루 24시간 값이 <== 고정된 값을 저장 할때 사용.
		
		List<Integer> aList7 = Arrays.asList(1,2,3,4);	//고정된 값을 저장 
		List<String> aList8 = Arrays.asList("월","화","수","목","금","토","일");
		aList7.set(1, 7); //set() : 기본의 값을 수정, index 1에 7을 수정 하겠다
//		aList7.add(5);	//오류 발생 .add() 마지막에서 값을 더 추가 할때 사용.
